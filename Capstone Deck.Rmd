...
subtitle: "... at least the next word"
author: "Edward Lee"
date: "April 20, 2016"
...
The method is based on using current words and phases in the websphere as training data for the predictive alogirthm for making the best guess at the next word following a sequence of input words

Zotar The Word Predictor is the result of the Capstone Project for the John Hopkins Coursera Data Science Specialization in partnership with Swiftkey.  The input was provided by Swiftkey for this application
...
- The Ngram datasets were then used as the foundation to perform word prediction
- R Shiny was used a the web front end for the word prediction application
...
- Ngram dataset consist of bigrams, trigrams, quadgrams, as well as 5grams.   I had a 6 grams model but decide to conserve load time by now using it
...
- If no match at the 5grams model, then move to the quadgrams model... and so forth
- The first matched model will be considered the best model and the most frequent word will be the "best" word

## Nuisances for the model

There are issues and decision made when creating the model, such as
